...
func (_q *PetQuery) QuerySkills() *PetSkillQuery {
	query := (&PetSkillClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *PetQuery) WithSkills(opts ...func(*PetSkillQuery)) *PetQuery {
	query := (&PetSkillClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Pet, error) {
	...
	if query := _q.withSkills; query != nil {
		if err := _q.loadSkills(ctx, query, nodes,
			func(n *Pet) { n.Edges.Skills = []*PetSkill{} },
			func(n *Pet, e *PetSkill) { n.Edges.Skills = append(n.Edges.Skills, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PetQuery) loadSkills(ctx context.Context, query *PetSkillQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *PetSkill)) error {
	...
	query.withFKs = true
	query.Where(predicate.PetSkill(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(pet.SkillsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.pet_skills
		if fk == nil {
			return fmt.Errorf(`foreign-key "pet_skills" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "pet_skills" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
